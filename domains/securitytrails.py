...
SECURITYTRAILS_KEY = os.environ['SECURITYTRAILS_TOKEN']
...
'''
lambda-function securitytrails-worker
'''
def worker(event, context):
    ...
    if len(results) > 0:
        output = bbrf('domain add '+' '.join(results)+' -p '+program + ' -s securitytrails')
...
def api_call(domain):
    ...
    r = requests.get(url, headers={'apikey': SECURITYTRAILS_KEY})
